module FireWall_Pack #(
    ...
    always @(*) begin
        next_state = state;
        case (state)
            IDLE:  if (user_ip_hi_wr_en) next_state = READ;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (user_ip_lo_wr_en) user_ip_lo <= reg_wdata;
            if (user_ip_hi_wr_en) user_ip_hi <= reg_wdata[16:0];
            new_entry <= {user_ip_hi, user_ip_lo};
            valid_bit<= user_ip_hi[16];
            case (state)
                READ: begin
                    pos          <= user_id[1:0];
